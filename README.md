...
### The Generated PIO Header

The PIO program (```include/pio-i2s-out.pio.h```) is assembled from ```src/pio-i2s-out.pio``` by pioasm and changes should be committed to source control.
Whenever the PIO source code is modified, it should be regenerated by running:

```sh
./update-pio-header.sh
```

The tests will fail if the source and generated header have drifted apart. To check without running the tests, run:

```sh
./update-pio-header.sh --check
```
